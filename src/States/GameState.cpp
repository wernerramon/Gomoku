...
void GameState::initSprites()
{
    ...
    m_line_hor_s = m_graphic_loader->loadSprite();
    ...
    if (m_light_mode)
    {
        ...
        m_line_hor_s->setTexture(m_line_light_hor_t, true);
    }
    else
    {
        m_bg_s->setTexture(m_bg_dark_t, true);
        m_line_hor_s->setTexture(m_line_dark_hor_t, true);
    }
    m_line_hor_s->setScale({scale_hor_line, 1});
    m_bg_s->setScale({scale_x, scale_y});
    m_line_hor_s->setPosition({0, 800});
}
